...
static void create_pin_code_screen(void)
{
    ...
    lv_obj_set_style_text_color(btnm, lv_color_hex(0xFFFFFF), LV_PART_ITEMS | LV_STATE_DEFAULT); // 设置按钮文字为白色
    lv_obj_set_style_shadow_width(btnm, 0, LV_PART_ITEMS | LV_STATE_DEFAULT);

    /* 特定按钮样式设置 */
    uint16_t del_btn_id = 11; // DEL按钮的ID
    uint16_t ok_btn_id = 13; // OK按钮的ID

    lv_btnmatrix_set_btn_ctrl(btnm, del_btn_id, LV_BTNMATRIX_CTRL_CUSTOM_1);
    lv_btnmatrix_set_btn_ctrl(btnm, ok_btn_id, LV_BTNMATRIX_CTRL_CUSTOM_1);

    /* 自定义样式 */
    static lv_style_t style_btn_special;
    lv_style_init(&style_btn_special);
    lv_style_set_bg_color(&style_btn_special, lv_color_hex(0x00FF00)); // 绿色背景

    /* 应用自定义样式 */
    lv_obj_add_style(btnm, &style_btn_special, LV_PART_ITEMS | LV_BTNMATRIX_CTRL_CUSTOM_1);
