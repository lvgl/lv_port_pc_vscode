...
#include "startup_import_abort.h"
#include "gui_comm.h"

extern void startup_screen_start(void);
extern void startup_import_wallet_start(void);

static startup_import_abort_t* p_startup_import_abort = NULL;
...
static void startup_import_abort_bg_cont(lv_obj_t* parent)
{
    ...
    lv_label_set_text(title_label, "Abort Import");
    ...
    lv_label_set_text(describe_label, "Are you sure to abort this process? All progress will be lost.");
    
    lv_obj_t* img_btn_confirm = lv_imagebutton_create(parent);
    lv_imagebutton_set_src(img_btn_confirm, LV_IMAGEBUTTON_STATE_RELEASED, &img_left_released_10e807_14x26, &img_mid_released_10e807_4x26, &img_right_released_10e807_14x26);
    lv_imagebutton_set_src(img_btn_confirm, LV_IMAGEBUTTON_STATE_PRESSED, &img_left_pressed_61fc5a_14x26, &img_mid_pressed_61fc5a_4x26, &img_right_pressed_61fc5a_14x26);
...
{
    p_startup_import_abort = (startup_import_abort_t*)lv_malloc(sizeof(startup_import_abort_t));
    LV_ASSERT(p_startup_import_abort);
    lv_memset(p_startup_import_abort, 0, sizeof(startup_import_abort_t));

    p_startup_import_abort->bg_cont = gui_comm_draw_bg();
    startup_import_abort_bg_cont(p_startup_import_abort->bg_cont);
}

void startup_import_abort_stop(void)
{
    lv_obj_del(p_startup_import_abort->bg_cont);
    lv_free(p_startup_import_abort);
    p_startup_import_abort = NULL;
}
