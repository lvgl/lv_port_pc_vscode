...
static guide_quick_start_t* p_guide_quick_start = NULL;
...
static void title_cb(lv_event_t* e)
{
    ...
    {
        guide_quick_start_stop();
        guide_set_pin_start();
    }
}

static void guide_create_wallet_event_handler(lv_event_t* e)
{
    lv_event_code_t event = lv_event_get_code(e);
    ...
    {
        printf("%s\n", __func__);
        //gui_app_run_subpage("guide", "ready_check", NULL);
        guide_quick_start_stop();
        guide_ready_check_start();
    }
}

static void guide_import_wallet_event_handler(lv_event_t* e)
{
    lv_event_code_t event = lv_event_get_code(e);
...
static void guide_quick_start_bg_cont(lv_obj_t* parent)
{
    guide_draw_title(parent, "Quick Start", title_cb);
    ...
    lv_obj_add_event_cb(img_btn_create, guide_create_wallet_event_handler, LV_EVENT_SHORT_CLICKED, NULL);
    ...
    lv_obj_add_event_cb(img_btn_import, guide_import_wallet_event_handler, LV_EVENT_SHORT_CLICKED, NULL);
...
{
    p_guide_quick_start = (guide_quick_start_t*)lv_malloc(sizeof(guide_quick_start_t));
    LV_ASSERT(p_guide_quick_start);
    lv_memset(p_guide_quick_start, 0, sizeof(guide_quick_start_t));

    p_guide_quick_start->bg_cont = guide_draw_bg_cont();
    guide_quick_start_bg_cont(p_guide_quick_start->bg_cont);
}

void guide_quick_start_stop(void)
{
    lv_obj_del(p_guide_quick_start->bg_cont);
    lv_free(p_guide_quick_start);
    p_guide_quick_start = NULL;
}
