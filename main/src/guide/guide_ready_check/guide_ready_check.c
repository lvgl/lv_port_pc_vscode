...
static guide_ready_check_t* p_guide_ready_check = NULL;
...
static void guide_btn_event_handler(lv_event_t* e)
{
    ...
    if (LV_EVENT_SHORT_CLICKED == event)
    {
        ...
        //gui_app_run_subpage("guide", "recovery", e->user_data);
        guide_ready_check_stop();
        guide_recovery_start();
    }
}

static void guide_ready_check_bg_cont(lv_obj_t* parent)
{
    guide_draw_title(parent, "Ready to Check", title_cb);
    ...
    lv_obj_add_event_cb(img_btn_words12, guide_btn_event_handler, LV_EVENT_SHORT_CLICKED, "12");
    ...
    lv_obj_add_event_cb(img_btn_words18, guide_btn_event_handler, LV_EVENT_SHORT_CLICKED, "18");
    ...
    lv_obj_add_event_cb(img_btn_words24, guide_btn_event_handler, LV_EVENT_SHORT_CLICKED, "24");
...
{
    p_guide_ready_check = (guide_ready_check_t*)lv_malloc(sizeof(guide_ready_check_t));
    LV_ASSERT(p_guide_ready_check);
    lv_memset(p_guide_ready_check, 0, sizeof(guide_ready_check_t));

    p_guide_ready_check->bg_cont = guide_draw_bg_cont();
    guide_ready_check_bg_cont(p_guide_ready_check->bg_cont);
}

void guide_ready_check_stop(void)
{
    lv_obj_del(p_guide_ready_check->bg_cont);
    lv_free(p_guide_ready_check);
    p_guide_ready_check = NULL;
}
